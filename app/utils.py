...
def process_excel(filename):
    try:
        logger.info(f"Lendo arquivo Excel: {filename}")
        ...
        logger.info(f"Arquivo processado: {processed_file}")
...
def translate_excel(filename):
    try:
        logger.info(f"Iniciando tradução do arquivo: {filename}")
        df = pd.read_excel(filename)
        ...
        logger.info(f"Arquivo traduzido: {translated_file}")
        return translated_file
    except Exception as e:
        logger.error(f"Erro ao traduzir o arquivo: {str(e)}")
        logger.error(traceback.format_exc())
        raise

def generate_summary(filename):
    try:
        logger.info(f"Gerando resumo para o arquivo: {filename}")
        df = pd.read_excel(filename)
        ...
        logger.info(f"Usando chave API: {openai.api_key[:5]}...")
        ...
        try:
            ...
            logger.error("Cota da API OpenAI excedida")
        ...
        logger.error(f"Erro ao gerar o resumo: {str(e)}")
        logger.error(traceback.format_exc())
        raise
